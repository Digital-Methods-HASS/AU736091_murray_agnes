...
date: 'created on 5 April 2024 and updated `r format(Sys.time(), "%d %B, %Y")`'
...
### Get the got report:
```{r get-document}
got.pdf_path <- here("data","got.pdf")

got_text <- pdf_text(got.pdf_path)
```
...
It seems reasonable that all these words are positive.
...
Which aligns with our expectations for Game of Thrones
...
ggplot(data = got_nrc_n, aes(x = sentiment, y = n)) +
  geom_col()
```
The two columns that spike are "negative" and "positive, a lot of words are in these categories because words without value, with a value of 0, will fit in both 


We can also count by sentiment *and* word, then facet:
```{r count-nrc}
got_nrc_n5 <- got_nrc %>% 
...
lord
```
We might consider adding "lord" to the stopword list since these sentiments seems conflicting

We'll do the same with "stark"
```{r nrc-stark}
